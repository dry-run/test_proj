...
migrate_multiple_repos() {
  ...
  while IFS= read -r REPO_NAME; do
    ./migrate-git-repos-base-ghes2ghec.sh $REPO_NAME $GHES_USER_NAME $GHES_USER_PASSWORD $GHES_ORG_NAME $GHES_HOST $GHEC_USER_NAME $GHEC_USER_PAT $GHEC_ORG_NAME $GHEC_MAIN_BRANCH
    MIGRATE_STATUS=$?
